...
import ModalComponent from "~/pages/adminBPP/baru/detail.vue";
          ...
          <tr v-for="(row, index) in rows" :key="index" class="text-sm text-gray-500 border">
            ...
            <td class="py-2 px-4 text-left">
              <button @click="openModal(row)"
                class="bg-[#0E9F6E] hover:bg-green-700 text-white py-1 px-4 rounded-lg">Detail</button>
            </td>
          </tr>
        </template>
      </Table>
...
const documents = computed(() => {
  const filteredDocs = dashboardStore.data ? dashboardStore.data.filter(doc => doc.status_baru_proposal === 'baru') : [];
