...
const regions = ref(['APBD', 'APBN', 'uang']); // Ganti dengan data wilayah yang sebenarnya
...
const filterDocuments = () => {
    ...
    if (typeof dateRange.value === 'string' && dateRange.value.includes('to')) {
        [startDate, endDate] = dateRange.value.split(' to ').map(date => new Date(date));
    } else if (Array.isArray(dateRange.value) && dateRange.value.length === 2) {
        [startDate, endDate] = dateRange.value.map(date => new Date(date));
    }

    if (startDate && endDate) {
        filteredDocuments.value = documents.value.filter(doc => {
            const docDate = new Date(doc.createdAt);
            const withinDateRange = docDate >= startDate && docDate <= endDate;
            const matchesRegion = selectedRegion.value ? doc.jenis_bantuan === selectedRegion.value : true;
            return withinDateRange && matchesRegion;
        });
    } else {
        filteredDocuments.value = documents.value.filter(doc =>
            selectedRegion.value ? doc.jenis_bantuan === selectedRegion.value : true
        );
    }
};


watch([dateRange, selectedRegion], filterDocuments);
  ...
  <MainLayoutBPP>
    <div class="w-full mt-20 text-black px-8">
      <div class="flex justify-end mb-4 space-x-4">
        ...
        <!-- Date Range Filter -->
        <div class="mb-4">
          <flat-pickr v-model="dateRange" :config="dateConfig" placeholder="Pilih Periode" class="w-max border-2 border-[#D1D5DB] rounded-lg text-center"></flat-pickr>
        </div>

        {{ dateRange }}
      </div>
      <Table :headers="tableHeader" :rows="filteredDocuments">
        <template #rows="{ rows }">
          <tr v-for="(row, index) in rows" :key="index" class="text-sm text-gray-500 border">
            ...
            <td class="py-2 px-4 text-left">
              <button @click="openModal(row)" class="bg-[#0E9F6E] hover:bg-green-700 text-white py-1 px-4 rounded-lg">Tinjau Ulang</button>
            </td>
          </tr>
        </template>
      </Table>
