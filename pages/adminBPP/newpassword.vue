<template>
  <section>
    <div
      class="h-screen w-screen flex items-center justify-center bg-cover"
        ...
        >
          <div class="w-full px-8 py-5">
            ...
            <div class="flex my-8 sm:mt-5">
              <div
                class="inline-flext flex-col items-center justify-center w-full"
              >
                <h3
                  class="text-sm font-semibold text-black leading-6 lg:text-xl text-center"
                >
                  Lupa Kata Sandi
                </h3>
              </div>
              ...
              <form @submit.prevent="handleSetNewPassword">
                ...
                <div class="flex flex-col mt-4 lg:space-y-2 py-6">
                  <button
                    type="submit"
                    name="setNewPassword"
                    class="flex items-center text-base justify-center mx-20 py-2 font-medium text-center text-white transition duration-500 ease-in-out transform bg-[#C53030] rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 lg:w-auto"
                  >
                    Setel Password Baru
                  </button>
                </div>
...
export default {
  setup() {
    ...
    const handleSetNewPassword = async () => {
      ...
      try {
        const response = await $forgetPasswordStore.setNewPassword(
          newPasswordData
        );
        console.log("New password set successfully");
        useNuxtApp().$toast.success(`Ubah sandi berhasil`, {
          autoClose: 2000,
        });
      } catch (error) {
